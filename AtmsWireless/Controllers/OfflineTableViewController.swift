...
class OfflineTableViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        APIManager.sharedInstance.getLists(username: "atmsth", password: "Anil", type: "all") { response in
            ...
            self.sections = Dictionary(grouping: self.down_data) { data -> Character in
...
extension OfflineTableViewController: UISearchBarDelegate, UISearchResultsUpdating, UISearchControllerDelegate {
        ...
        if searchBar.text != "" {
            ...
            self.sections.map {
                let filtered = $0.datas.filter {
                    ...
                    return description.lowercased().contains(searchBar.text!.lowercased()) || serial_number.lowercased().contains(searchBar.text!.lowercased())
                }
                if filtered.count > 0 {
